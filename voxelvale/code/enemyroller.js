...
class Enemy {
	constructor(X=null,Y=null,Z=null){
		...
		this.lowerBound = vec4(0,0,0,1);
		this.upperBound = vec4(1,1,1,1);
	}
	...
	sendHitboxData(){
		push_wireframe_indices(this.lowerBound,this.upperBound);
	}

	initialize_enemy(){}

	returnBounds(){
		return [mult(translate(this.posX, this.posY,this.posZ),this.lowerBound), mult(translate(this.posX, this.posY,this.posZ),this.upperBound)];
	}

	//Updates the position of the enemy prior to drawing it.
	updatePosition(){
		this.instanceMat = translate(this.posX,this.posY,this.posZ);
	}
...
class Finder extends Enemy{
	...
	getCandidates(square, grid){
		...
		//Diagonals (add after, can't move through blocks!)

		//Bottom left
		if(PX - 1 >= 0 && PY - 1 >= 0){
			if(!grid[PX-1][PY-1] && !grid[PX-1][PY] && !grid[PX][PY-1])
				candidates.push([PX-1,PY-1]);
		}

		//Bottom right
		if(PX + 1 < this.roomSizeX && PY - 1 >= 0){
			if(!grid[PX+1][PY-1] &&!grid[PX+1][PY] && !grid[PX][PY-1])
				candidates.push([PX+1,PY-1]);
		}

		//Top left
		if(PX - 1 >= 0 && PY + 1 < this.roomSizeY){
			if(!grid[PX-1][PY+1] && !grid[PX][PY+1] && !grid[PX-1][PY])
				candidates.push([PX-1,PY+1]);
		}

		//Top right
		if(PX + 1 < this.roomSizeX && PY + 1 < this.roomSizeY){
			if(!grid[PX+1][PY+1] && !grid[PX+1][PY] && !grid[PX][PY+1])
				candidates.push([PX+1,PY+1]);
		}
...
/*
	Todo: Add spikes (maybe).

	As a note, the roller is always on level z = -3.
	Also this is an enemy that can only appear in dungeons. So no world, just "currentDungeon".
*/
class Roller extends Enemy{
	constructor(X=null,Y=null,Z=null){
		...
		this.rollingDirection = RIGHT;
	}

	//Basically, the roller is just a cube that rolls
	sendData(){
		build_block(0);
	}

	rollHorizontal(){
		return mult(translate(0.5,0.5,0.5),mult(rotateY(this.rollY),translate(-0.5,-0.5,-0.5)));
	}
	rollVertical(){
		return mult(translate(0.5,0.5,0.5),mult(rotateX(this.rollX),translate(-0.5,-0.5,-0.5)));
	}

	updatePosition(){
		/*
			Check collisions and change rolling direction.
		*/
		var PX = Math.round(this.posX);
		var PY = Math.round(this.posY);
		var PZ = this.posZ;

		var curBlock = currentDungeon.getIsOccupied(PX,PY,PZ);
		switch(this.rollingDirection){
			case RIGHT:
				var rightBlock = currentDungeon.getIsOccupied(PX+1,PY,PZ);
				if(curBlock || rightBlock){
					if(isColliding(this,currentDungeon.getBlockAt(PX,PY,PZ)) || isColliding(this,currentDungeon.getBlockAt(PX+1,PY,PZ))){
						this.rollingDirection = TOP;
					}
				}
				break;
			case LEFT:
				var leftBlock = currentDungeon.getIsOccupied(PX-1,PY,PZ);
				if(curBlock || leftBlock){
					if(isColliding(this,currentDungeon.getBlockAt(PX,PY,PZ)) || isColliding(this,currentDungeon.getBlockAt(PX-1,PY,PZ))){
						this.rollingDirection = BOTTOM;
					}
				}
				break;
			case TOP:
				var topBlock = currentDungeon.getIsOccupied(PX,PY+1,PZ);
				if(curBlock || topBlock){
					if(isColliding(this,currentDungeon.getBlockAt(PX,PY,PZ)) || isColliding(this,currentDungeon.getBlockAt(PX,PY+1,PZ))){
						this.rollingDirection = LEFT;
					}	
				}
				break;
			case BOTTOM:
				var bottomBlock = currentDungeon.getIsOccupied(PX,PY-1,PZ);
				if(curBlock || bottomBlock){
					if(isColliding(this,currentDungeon.getBlockAt(PX,PY,PZ)) || isColliding(this,currentDungeon.getBlockAt(PX,PY-1,PZ))){
						this.rollingDirection = RIGHT;
					}	
				}
		...
		switch(this.rollingDirection){
			case RIGHT:
				...
				this.rollY-=this.rotSpeed;
				this.instanceMat = mult(translate(this.posX,this.posY,this.posZ),this.rollHorizontal());
				break;
			...
			case TOP:
				...
				this.rollX+=this.rotSpeed;
				this.instanceMat = mult(translate(this.posX,this.posY,this.posZ),this.rollVertical());
				break;
